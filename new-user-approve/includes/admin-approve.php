...
class PW_New_User_Approve_Admin_Approve {
	...
	public function approve_admin() {
		include_once pw_new_user_approve()->get_plugin_dir() . '/admin/templates/approve.php';
	}
...
$SERVER_URI = sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI'])); 
				...
				?>
					...
					<td><a href="mailto:<?php echo esc_attr($user->user_email); ?>"
						   title="<?php esc_attr_e('email:', 'new-user-approve'); ?> <?php echo esc_attr($user->user_email); ?>"><?php echo esc_attr($user->user_email); ?></a>
					</td>

					<td class="actions-btn">
						<?php if ($approve && $user->ID != get_current_user_id()) { ?>
							<span><a class="button approve-btn" href= "<?php echo esc_url($approve_link); ?>" title="<?php esc_attr_e('Approve', 'new-user-approve'); ?> <?php echo esc_attr($user->user_login); ?>"><?php echo esc_html('Approve', 'new-user-approve'); ?></a> </span>
						<?php } ?>

						<?php if ($deny && $user->ID != get_current_user_id()) { ?>
							<span><a class="button deny-btn" href="<?php echo esc_url($deny_link); ?>" title="<?php esc_attr_e('Deny', 'new-user-approve'); ?> <?php echo esc_attr($user->user_login); ?>"><?php echo esc_html('Deny', 'new-user-approve'); ?></a></span>
						<?php } ?>
			...
			// translators: %s is for user status
	...
	public function admin_notice() {
		...
		if ($screen->id == 'users_page_new-user-approve-admin') {
			...
			if (!get_user_meta($user_id, 'pw_new_user_approve_ignore_notice')) {
				?>
				<div class="updated"><p>
				<!-- translators: %1$s is for user admin url page and %2$s is for hide notice -->
				<?php printf(wp_kses_post('You can now update user status on the <a href="%1$s">users admin page</a>. | <a href="%2$s">Hide Notice</a>', 'new-user-approve'), esc_url( admin_url('users.php')), esc_url(add_query_arg(array( 'new-user-approve-ignore-notice' => 1 )))); ?>
				 </p></div>
				<?php
			}
		}
...
<?php
	...
	public function zapier() {
		...
		<?php $nonce = wp_create_nonce('api-generate-nonce'); ?>
						<?php $get_api_key = !empty( $get_api_key ) ? $get_api_key : ''; ?>
						<input type='text' name='nua_api_key' id='nua-api' value = "<?php echo esc_attr($get_api_key); ?>"  />
						<button id='nua-generate-api' class='button'><?php esc_html_e('Generate API Key', 'new-user-approve'); ?></button>
						<input type='hidden' name='wp-api-generate-nonce' value='<?php echo esc_attr( $nonce ); ?>' />
		...
		<ul style='padding-left: 30px; list-style-type: disc;'>
		<?php foreach ($nua_triggers as $nua_trigger) : ?>
					<?php // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText ?>
				  <li> <?php esc_html_e($nua_trigger, 'new-user-approve'); ?> </li>
