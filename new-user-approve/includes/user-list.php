...
class PW_New_User_Approve_User_List {
	...
	public function admin_notices() {
		...
		if (isset($_REQUEST['denied']) && (int) $_REQUEST['denied'] ) {
			...
			// translators: %s is for denied status
		...
		if (isset($_REQUEST['approved']) && (int) $_REQUEST['approved'] ) {
			...
			// translators: %s is for approved status
...
return;
	...
	public function pending_users_bubble() {
		...
		if ($pending_users > 0 ) {
			...
			// Modify menu item
			// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Ignoring global variable override in this section
