...
class PW_New_User_Approve {
	...
	private function includes() {
		include_once $this->get_plugin_dir() . 'includes/email-tags.php';
		include_once $this->get_plugin_dir() . 'includes/messages.php';
		include_once $this->get_plugin_dir() . 'includes/invitation-code.php';
		include_once $this->get_plugin_dir() . 'includes/zapier/zapier.php';
	}
	...
	public function activation() {
		...
		// translators: %s is for minimum version of wordpress for new user approve
	...
	public function admin_notices() {
		...
		if (!$show_notice ) {
			...
			printf(
				wp_kses_post('The Membership setting must be turned on in order for the New User Approve to work correctly. <a href="%1$s">Update in settings</a>. | <a href="%2$s">Hide Notice</a>'), esc_url(admin_url('options-general.php')), esc_url(
					add_query_arg(
						array(
	...
	public function admin_loaded() {
		...
		if ($user_admin_integration ) {
			include_once __DIR__ . '/includes/user-list.php';
		}
		$legacy_panel = apply_filters('new_user_approve_user_admin_legacy', true);
		if ($legacy_panel ) {
			include_once __DIR__ . '/includes/admin-approve.php';
		}
	}
	...
	public function nua_admin_scripts() {
		...
		if (isset($_GET['page']) && in_array($_GET['page'], $pages)) {
			...
			wp_enqueue_script('nua-admin', plugins_url('/assets/js/admin.js', __FILE__), array( 'jquery' ), NUA_VERSION, true);
			wp_localize_script(
				'nua-admin',
				'nuaAdmin',
				array(
	...
	public function admin_approval_email( $user_login, $user_email ) {
		...
		// translators: [%s] is for blogname in subject of admin approval request email
	...
	public function create_new_user( $user_login, $user_email, $errors ) {
		...
		if (!$user_id ) {
			// translators: %s is for blogname in error of user registration
			$errors->add('registerfail', sprintf(__('<strong>ERROR</strong>: Couldn&#8217;t register you... please contact the <a href="mailto:%s">webmaster</a> !', 'new-user-approve'), get_option('admin_email')));
		} else {
			...
			if (false===$disable) {
				...
				// translators: [%s] is for blogname in subject of approved email - wp default
	...
	public function approve_user( $user_id ) {
		...
		// translators: [%s] is for blogname in subject of approved email 
	...
	public function deny_user( $user_id ) {
		...
		// translators: [%s] is for blogname in subject of denied email 
...
if (nua_fs()->is__premium_only() && nua_fs()->can_use_premium_code() ) {
	$filename = __DIR__ . '/premium-files/new-user-approve-options.php';
