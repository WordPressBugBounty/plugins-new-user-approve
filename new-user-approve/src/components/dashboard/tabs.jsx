...
const NUA_Dash_Tabs = () => {
  ...
  let users_icon = (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M6.57757 15.1335C5.1628 15.8917 1.45336 17.4398 3.71266 19.377C4.81631 20.3233 6.04549 21.0001 7.59087 21.0001H16.4091C17.9545 21.0001 19.1837 20.3233 20.2873 19.377C22.5466 17.4398 18.8372 15.8917 17.4224 15.1335C14.1048 13.3556 9.89519 13.3556 6.57757 15.1335Z" fill="#242424" />
      <path d="M16.5 7.5C16.5 9.98528 14.4853 12 12 12C9.51472 12 7.5 9.98528 7.5 7.5C7.5 5.01472 9.51472 3 12 3C14.4853 3 16.5 5.01472 16.5 7.5Z" fill="#242424" />
    </svg>
  );
  ...
  let qr_button_icon = (
    <svg width="24" height="24" viewBox="0 0 65 65" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M45.9999 5.41174H18.9411C15.9565 5.41174 13.5293 7.83892 13.5293 10.8235V54.1176C13.5293 57.1022 15.9565 59.5294 18.9411 59.5294H45.9999C48.9845 59.5294 51.4117 57.1022 51.4117 54.1176V10.8235C51.4117 7.83892 48.9845 5.41174 45.9999 5.41174ZM18.9411 45.9973V13.5294H45.9999L46.0053 45.9973H18.9411Z" fill="#242424"/>
    </svg>
  );
  ...
  return (
    <Tabs className="nua_dash_parent_tabs" defaultValue={"dashboard"} value={currentTab} onChange={handleTabChange}>
      <TabsList className="nua_dash_parent_tablist">
        <Tab value={"dashboard"} component={Link} to={"/dashboard"} className={currentTab == "dashboard" ? "nua_active_tab" : ""} datatarget={"dashboard"}>
          <span>
            {dash_icon}
        ...
        {nuaAdmin.nua_users_cap && (
          <Tab value={"action=users"} component={Link} to={"/action=users"} className={currentTab == "action=users" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=users/tab=all-users")} datatarget={"users"}>
            <span>
              {users_icon}
        ...
        {nuaAdmin.nua_view_invitation_tab && (
          <Tab value={"action=inv-codes"} component={Link} to={"/action=inv-codes"} className={currentTab === "action=inv-codes" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=inv-codes/tab=all-codes")} datatarget={"inv code"}>
            <span>
              {invitation_icon}
        ...
        {nuaAdmin.nua_auto_approve_cap && (
          <Tab value={"action=auto-approve"} component={Link} to={"/action=auto-approve"} className={currentTab == "action=auto-approve" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=auto-approve/tab=whitelist")} datatarget={"auto approve"}>
            <span>
              {auto_approve_icon}
        ...
        {nuaAdmin.nua_integration_cap && (
          <Tab value={"action=integrations"} component={Link} to={"/action=integrations"} className={currentTab == "action=integrations" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=integrations")}>
            <span>
              {integration_icon}
        ...
        {nuaAdmin.nua_mobile_app_cap && (
          <Tab value={"action=mobile-app"} component={Link} to={"/action=mobile-app"} className={currentTab == "action=mobile-app" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=mobile-app")}>
            <span>
              {qr_button_icon}
              {__("Mobile App", "new-user-approve")}
              <span className="nua-app-new-badge"> {__("New", "new-user-approve")}</span>
            </span>
          </Tab>
        )}
        {nuaAdmin.nua_settings_cap && (
          <Tab value={"action=settings"} component={Link} to={"/action=settings"} className={currentTab == "action=settings" ? "nua_active_tab" : ""} onClick={() => (window.location.hash = "#/action=settings/tab=general")}>
            <span>
              {setting_icon}
      ...
      {nuaAdmin.nua_integration_cap && (
        <TabPanel className="integration-main-tabpanel" value={"action=integrations"} index="integrations">
          <Integrations />
        </TabPanel>
      )}
      {nuaAdmin.nua_mobile_app_cap && (
        <TabPanel className="mobileapp-main-tabpanel" value={"action=mobile-app"} index="mobile-app">
          <Mobile_App />
        </TabPanel> 
      )}
      {nuaAdmin.nua_settings_cap && (
        <TabPanel className="setting-main-tabpanel generalTabPage" value={"action=settings"} index="settings">
          <Settings />
        </TabPanel>
      )}
    </Tabs>
